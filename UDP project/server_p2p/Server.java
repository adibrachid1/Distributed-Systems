  ...
  public static void main(String args[]){
    ...
    try{
      ...
      while(true){
        ...
        message_received=new String(request.getData()).trim();//order received from client
        ...
        show_users();
          ...
          System.out.println("Connected users: ");
          show_users();
        }else if(message_received.trim().equals("connectto")){
          buffer=new byte[1024];
          ...
          String[] parts = message_received.split("-");
          String id_rec = parts[1]; 
          port_rec=Integer.parseInt(id_rec);
          String msg = Integer.toString(get_id(Integer.parseInt(parts[2])))+"-"+parts[0];
          buffer = new byte[1024];
          buffer =msg.getBytes();
          reply = new DatagramPacket(buffer,  buffer.length, request.getAddress(),port_rec);
          aSocket.send(reply);//send the port number of the 2nd client
          System.out.println("Server sending message "+parts[0]+" from User "+get_id(request.getPort())+" to "+get_id(port_rec));
        }      
      }
