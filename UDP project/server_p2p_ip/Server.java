...
public class Server{
    ...
    try{
      ...
      while(true){
        ...
        if(message_received.trim().equals("connect")){
          id++;
          User a=new User(id,request.getPort(),request.getAddress());
          users.add(a);
          System.out.println("User"+id+" joined - port:"+request.getPort()+" - ip:"+request.getAddress());
          ...
          while(itr.hasNext()){  
            a=itr.next();
            reply = new DatagramPacket(buffersend,buffersend.length,a.get_address(), a.get_port());
            aSocket.send(reply);
          }
  ...
  public static String get_connected_users(){
    ...
    while(itr.hasNext()){  
      a=itr.next();
      r+="-"+Integer.toString(a.id)+" &ip:"+a.ip;  
    } 
    return r;
  }
...
class User{
  ...
  InetAddress ip;
  int port;
  public User(int i,int p,InetAddress ip){
    id=i;
    this.ip=ip;
    port=p;
  }
